...
app.get('/', restrict, staff, function (req, res) {
  ...
  api.candidate.search(query, function(err, candidates) {
    if (err) return _handleError(err, req, res);

    log('Found %d published candidates', candidates.length);

    var publishedCandidates = candidates.length;
    ...
    api.user.all(function (err, users) {
      ...
      api.user.findEmailValidated(function(err, users) {
        ...
        api.candidate.votes(function (err, votes) {
          if (err) return _handleError(err, req, res);
