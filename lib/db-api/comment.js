...
exports.getSideComments = function getSideComments (candidateId, paging, fn) {
    log('Querying side comments for candidate %s', candidateId);
    paging = paging || {};

    Comment
        .find({ candidateId: candidateId, context: 'paragraph' })
...
exports.reply = function reply(commentId, reply, fn) {
    ...
    Comment.findById(commentId, function(err, comment) {
        ...
        Candidate.get(comment.reference, function(err, candidate) {
            if (!err && candidate && candidate.candidateUser) {
                var candUserId = candidate.candidateUser._id ? candidate.candidateUser._id : canidate.candidateUser;
                var authorId = reply.author._id ? reply.author._id : reply.author;
                reply.selfComment = (candUserId == authorId);
            } else {
                reply.selfComment = false;
            }
            ...
            comment.save(function(err, saved) {
                ...
                User.populate(comment, opts, function(err, comment) {
                    ...
                    if (comment.author != reply.author.id) {
                        ...
                        var candidateUrl = '';
                        candidateUrl = utils.buildUrl(false, { pathname: '/candidate/' + comment.reference });
                        ...
                        notifier.notify(eventName)
                            ...
                            .withData({ reply: r, comment: c, url: candidateUrl })
