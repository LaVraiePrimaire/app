...
exports.all = function all(params, fn) {
  ...
  Candidate
  .find(query)
  .select('id candidateId mediaTitle participants votes createdAt updatedAt closingAt publishedAt deletedAt status open closed links author authorUrl')
...
exports.rss = function all(fn) {
  ...
  Candidate
  .find({ deletedAt: null })
  .select('id candidateId mediaTitle publishedAt body')
  .exec(function (err, candidates) {
    if (err) {
      log('Found error %j', err);
      return fn(err);
