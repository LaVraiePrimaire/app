...
exports.all = function all(params, fn) {
  ...
  Candidate
  .find(query)
  .select('id firstName lastName fullName nominatorName pictureUrl whatIsLife secondChartePos thirdChartePos fourthChartePos fifthChartePos firstProp secondProp thirdProp postal email phone participants votes createdAt updatedAt closingAt publishedAt deletedAt acceptedNominationAt status open closed acceptedNomination links candidateUser nominatorUser')
...
function createCandidate(data, fn) {
  data = populateCandidateNominatorUser(data);
...
function populateCandidateNominatorUser(data) {
  ...
  var nominatorUserEmail = data.nominatorUserEmail;
  delete data.nominatorUserEmail;
  if (nominatorUserEmail) {
      User.findOne({ email: nominatorUserEmail }, function(err, user) {
          if (err) return fn(err);
          data.nominatorUser = user;
      });
  }
  return data;
}
