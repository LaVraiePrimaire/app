...
exports.all = function all(params, fn) {
  ...
  Candidate
  .find(query)
  .select('id candidateId firstName lastName fullName postal email phone participants votes createdAt updatedAt closingAt publishedAt deletedAt acceptedNominationAt status open closed acceptedNomination links')
...
exports.vote = function vote(id, user, value, fn) {
  ...
  log('Proceding to vote %s at candidate %s by user %s', value, id, user.id || user);
  ...
  .exec(function (err, candidate) {
    ...
    doVote(candidate, user, value, fn);
  });
};

function doVote(candidate, user, value, cb) {
  candidate.vote(user.id, value, function (err) {
    if (err) {
      log('Found error %s', err);
      return cb(err);
    }

    log('Voted %s at candidate %s by user %s', value, candidate.id, user.id || user);
    cb(null, candidate);
  });
}
...
exports.rss = function all(fn) {
  ...
  Candidate
  .find({ deletedAt: null })
  .select('id candidateId fullName publishedAt body')
  .exec(function (err, candidates) {
    if (err) {
      log('Found error %j', err);
      return fn(err);
