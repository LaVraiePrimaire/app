      ...
      db.users.findOne({ _id: ObjectId(data.reply.author.id) }, function(err, replyAuthor) {
        ...
        var params = {
          ...
          vars: [
            ...
            { name: 'REPLIER_NAME', content: replyAuthor.firstName + ' ' + replyAuthor.lastName },
            { name: 'COMMENT_TEXT', content: data.comment.text },
            { name: 'REPLY_TEXT', content: data.reply.text },
