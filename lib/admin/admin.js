...
import UsersListView from '../admin-users/view';
...
import { findUsers } from '../user-middlewares/user-middlewares';
...
page('/admin/users/', findUsers, ctx => {
  let currentPath = ctx.path;
  let usersList = new UsersListView(ctx.users);
  usersList.replace('.admin-content');
  ctx.sidebar.set('users');
  usersList.switchOn();
/*
  const onUsersUpdate = () => { page(currentPath); };
  bus.once('user-store:update:all', onUsersUpdate);
  bus.once('page:change', () => {
    bus.off('user-store:update:all', onUsersUpdate);
  });
*/
});
