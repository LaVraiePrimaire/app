...
page('/admin/topics/create', clearTagStore, findAllTags, ctx => {
  ctx.sidebar.set('topics');
...
page('/admin/topics/:id', clearTagStore, findAllTags, findTopic, ctx => {
...
page('/admin/tags', clearTagStore, findAllTags, ctx => {
...
page('/admin/tags/create', ctx => {
  let form = new TagForm();
  form.replace('.admin-content');
  ctx.sidebar.set('tags');
});

page('/admin/tags/:id', findTag, ctx => {
  // force section for edit
  // as part of list
  ctx.sidebar.set('tags');
