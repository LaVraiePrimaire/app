/**
 * Module dependencies.
 */

var express = require('express');
var utils = require('lib/utils');
var restrict = utils.restrict;
var log = require('debug')('democracyos:admin');
var t = require('t-component');
var config = require('lib/config');

/**
 * Exports Application
 */

var app = module.exports = express();

app.get('/admin', require('lib/layout'));
app.get('/admin/topics', require('lib/layout'));
app.get('/admin/topics/:id', require('lib/layout'));
app.get('/admin/topics/create', require('lib/layout'));
app.get('/admin/tags', require('lib/layout'));
app.get('/admin/tags/:id', require('lib/layout'));
app.get('/admin/tags/create', require('lib/layout'));
app.get('/admin/users', require('lib/layout'));
app.get('/admin/users/create', require('lib/layout'));
app.get('/admin/users/:id', require('lib/layout'));
