...
export default class ProposalClauses extends View {
  ...
  constructor (candidate) {
    super(template, { clauses: candidate.clauses });
  ...
  load () {
    ...
    request
      .get(`/api/candidate/${this.candidate.id}/sidecomments`)
      .end((err, res) => {
  ...
  getClausesSections () {
    ...
    function filterBySection(index) {
      return function (comment) {
        return comment.reference == candidate.id + '-' + index;
      }
    }

    candidate.clauses.forEach(function (clause) {
      let section = {};
  ...
  onClauseCommentPosted (comment) {
    request
      .post(`/api/candidate/${comment.sectionId}/comment`)
      .send({
