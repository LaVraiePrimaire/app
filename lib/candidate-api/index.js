...
var migrateCandidate = require('lib/migrations/candidate');
...
var candidateListKeys = [
  'id candidateId firstName lastName postal email phone postal fullName acceptedNomination status open closed public draft deleted',
  'participants voted createdAt updatedAt closingAt acceptedNominationAt',
  'publishedAt candidateUser deletedAt votable clauseTruncationText links'
].join(' ');

var candidateKeys = candidateListKeys
              + ' '
              + 'summary clauses state upvotes downvotes abstentions';
...
app.get('/candidate/:id', function (req, res) {
  ...
  api.candidate.get(req.params.id, function (err, candidate) {
    ...
    migrateCandidate(candidate, function (err, candidateMigrated) {
      if (err) {
        return _handleError(err, req, res);
      }

      log('Serving candidate %s', candidateMigrated.id);
      res.json(exposeCandidate(candidateMigrated, req.user));
    });
  });
});
...
app.post('/candidate/create', restrict, hasAccess, function (req, res, next) {
  ...
  api.candidate.create(req.body, function (err, candidate) {
    ...
    var keys = [
      'id candidateId firstName lastName fullName postal email phone body clauses state',
      'status open closed public draft deleted acceptedNomination participants',
      'upvotes downvotes abstentions createdAt updatedAt closingAt acceptedNominationAt',
      'publishedAt deletedAt votable bodyTruncationText links candidateUser'
    ].join(' ');
    res.json(exposeCandidate(candidate, req.user));
  });
});
...
app.post('/candidate/:id/publish', restrict, hasAccess, function (req, res) {
  ...
  api.candidate.get(req.params.id, function (err, candidate) {
    ...
    candidate.save(function (err, saved) {
      ...
      var candidateUrl = utils.buildUrl(config, { pathname: '/candidate/' + candidate.id });

      var data = {
        candidate: { mediaTitle: candidate.fullName, id: candidate.id },
        url: candidateUrl
      };
